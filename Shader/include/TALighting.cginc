...
struct PbrData
{
    float3 baseColor; // base (diffuse or specular) color
    float3 normal; // tangent space normal, if written
    float3 emission;
    float metallic; // 0=non-metal, 1=metal
    float roughness; // 0=rough, 1=smooth
    float ao; // occlusion (default 1)
    float alpha;
};
...
/*
Vertex2FragmentData TALightingVertex(MeshData v)
{
    Vertex2FragmentData o = (Vertex2FragmentData)0;
...
float4 GodPBR(in VertexData vertexData, in PbrData pbrData)
{
    //Variable
    float3 T = normalize(vertexData.tangent);
    float3 N = normalize(vertexData.normal);
    float3 B = normalize(cross(N, T));
    float3 L = normalize(UnityWorldSpaceLightDir(vertexData.worldPosition.xyz));
    float3 V = normalize(UnityWorldSpaceViewDir(vertexData.worldPosition.xyz));
    float3 H = normalize(V + L);

    //================== Normal Map  ============================================== //
    float3 NormalMap = pbrData.normal;
    ...
    float4 FinalColor = 0;
    //================== PBR  ============================================== //
    float3 BaseColor = pbrData.baseColor;
    float Roughness = pbrData.roughness;
    float Metallic = pbrData.metallic;

    float3 Emission = pbrData.emission;
    float3 AO = pbrData.ao;
